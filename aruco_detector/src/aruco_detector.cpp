...
namespace Aruco
{
    ...
    std::pair<std::vector<std::vector<cv::Point2f>>, int> ArucoDetector::secondary_detection(std::vector<std::vector<cv::Point2f>> rejected)
    {
        ...
        rejected.erase(std::remove_if(rejected.begin(), rejected.end(), [](std::vector<cv::Point2f> i)
                                      { return cv::contourArea(i) < 500; }),
                       rejected.end());

        // order in descending order of area
        std::sort(rejected.begin(), rejected.end(), [](std::vector<cv::Point2f> i, std::vector<cv::Point2f> j)
