...
namespace ariitk::state_machine
{
    ...
    double hover_height, land_height, step_height;
    double HOVER_TIME = 5.0, land_time;
    double DELAY_TIME = 0.5;

    int8_t ID_AR_LZ = 0, ID_AR_BOX = 5, ID_CLR_BOX = 75, ID_AR_DZ = 10;

    double TRANSITION_TIME = 5.0;

    ros::Rate LOOP_RATE(10);

    std_msgs::UInt16 lidar_dist_;
    geometry_msgs::PoseStamped mav_pose_;
    shastra_msgs::TagPose tag_pose_;
    shastra_msgs::UTMPose utm_pose_, lz_pose_;
    mavros_msgs::State mav_mode_;
    mavros_msgs::WaypointReached prev_wp;

    string mission_info = "mission_info", emag_control = "emag/control", odometry = "odometry", state = "state", utm_pose = "utm_pose", set_mode = "set_mode", mission_waypoint_pull = "mission/wpPull", pose_estimator = "pose_estimator/aruco_pose", lidar_distance = "lidar/distance/distance_raw", land = "mavros/cmd/land", mission_reached = "mission/reached";

    /*
            State Variables
    */
    bool CONTINUE_MISSION = true;
    bool BOX_ATTACHED = true;
    bool AT_LZ = false;
    ...
    */
    ...
    /*
        publishers
    */
    ros::Publisher command_pub_ = NH.advertise<geometry_msgs::PoseStamped>(mission_info, 10);
    ros::Publisher emag_pub_ = NH.advertise<std_msgs::UInt8>(emag_control, 5);

    /*
        subscribers
    */
    ros::Subscriber mav_pose_sub_ = NH.subscribe(odometry, 10, mav_pose_cb_);
    ros::Subscriber loc_pose_sub_ = NH.subscribe(pose_estimator, 1, pose_estimator_cb_);
    ros::Subscriber lidar_dist_sub_ = NH.subscribe(lidar_distance, 5, lidar_dist_cb_);
    ...
    ros::Subscriber mission_wp_sub = NH.subscribe(mission_reached, 10, wp_reached_cb_);

    /*
        service clients
    */
    ros::ServiceClient set_mode_client = NH.serviceClient<mavros_msgs::SetMode>(set_mode);
    ros::ServiceClient mission_client = NH.serviceClient<mavros_msgs::WaypointPull>(mission_waypoint_pull);
    ros::ServiceClient landing_client = NH.serviceClient<mavros_msgs::CommandTOL>(land);
